...
impl Part1 for Day1 {
    fn part1(&self, input: &str) -> usize {
        ...
        first.sort();
        first.iter().fold(0,|acc, f| acc+last.remove(last.iter().position(|x|x==last.iter().min().unwrap()).unwrap()).abs_diff(*f))
    }
}
...
impl Part2 for Day1 {
    fn part2(&self, input: &str) -> usize {
        ...
        first.iter().fold(0, |acc, f| acc+f*last.iter().filter(|n| n==&f).count())
    }
}
